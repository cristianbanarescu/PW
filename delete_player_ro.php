...
if(!empty($_POST["form_delete"]))
{
	...
	$delete = "DELETE FROM `player` WHERE nume_jucator='$var_numeJucator'";
		if (!mysqli_query($link, $delete)) echo "<script type='text/javascript'>alert('Eroare stergere jucator');</script>";
		else
			echo "<script type='text/javascript'>alert('Jucator sters');</script>";
}
